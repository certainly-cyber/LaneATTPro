...
# 这里目前的算法是只看两个y坐标
def jhnms(boxes, scores, overlap=0.7, top_k=200):
    """
    输入:
    ...
    while idx.numel() > 0:
        ...
        torch.index_select(y3, 0, idx, out=yy3)  # torch.index_select（） # 从tensor中按指定维度和索引 取值；
        # torch.index_select(y4, 0, idx, out=yy4)
        # torch.index_select(y5, 0, idx, out=yy5)  # torch.index_select（） # 从tensor中按指定维度和索引 取值；
        # torch.index_select(y6, 0, idx, out=yy6)
        # 计算当前最大置信框与其他剩余框的交集，不知道clamp的同学确实容易被误导,对于直线不需要
        # yy1 = torch.clamp(yy1, max=y1[i])  # max(y1,yy1)
        ...
        IoU = abs(yy1-y1[i])\
              ...
              +abs(yy3-y3[i])
              # +abs(yy4-y4[i])
              # +abs(yy5-y5[i])+abs(yy6-y6[i])
        IoU = IoU/3
        # print(IoU.numpy())
        # 选出IoU <= overlap的boxes(注意le函数的使用)
        idx = idx[IoU.ge(overlap)]  # le: 小于等于 返回的bool ， 去除大于overlap的值；，ge大于等于
        ...
        for proposals, attention_matrix in zip(batch_proposals, batch_attention_matrix):
            ...
            with torch.no_grad():
                ...
                if proposals.shape[0] == 0:
                    ...
                    continue
                if choose.cpu_flag:
                    keep, num_to_keep = jhnms(proposals, scores, 11, 20)
                else:
